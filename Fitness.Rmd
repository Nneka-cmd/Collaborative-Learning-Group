...
SP$addTraitA(mean=c(10,10), var=c(1,1), nQtlPerChr=20)

SP$setVarE(H2=c(0.5, 0.5))
#SP$addSnpChip(nSnpPerChr=1)
foundingPop <- newPop(founders, simParam = SP)
#snps <- pullSnpGeno(newPop, simParam = SP)

#plot3dPopulationFitness(foundingPop, calculateFitnessTwoTrait)
#plotTraitArchitecture(pop=foundingPop, fitFunc=twoTraitFitFunc)
```
...
for (gen in 1:nBurnInGens) {
  ...
  fit_df$traitValA[gen] <- meanP(pop)[1]
  fit_df$traitValB[gen] <- meanP(pop)[2]
  pop <- selectCross(pop, trait=twoTraitFitFunc, nInd=nInd(pop)*0.9, nCrosses=nInd(pop))
}
...
for (s in 1:nSims) {
  ...
  popA <- selectInd(pop, use="rand", nInd=popSize*0.2)
  popB <- selectInd(pop, use="rand", nInd=popSize*0.2)
  idx <- 1
  inc <- FALSE
  ...
  for (gen in 1:nSegGens) {
    ...
    if (trackFixedAlleles) {
      prevGeno <- pullSegSiteGeno(prevPop)
      newGeno <- pullSegSiteGeno(popA)
