...
pop_vec <- c(50,100,150,200,250,300,500,700,800,1000,1200,1400,1600,1800,2000)
  ...
  for (s in 1:length(sel_int_vec)) {
    ...
    for (sim in c(1:nSims)) {
      ...
      for(gen in 1:nGens) {
        ...
        pop <- selectCross(pop=pop, use=fitnessFunc, nInd=popSize*selInt, nCrosses = popSize)
      }
...
SP$addTraitA(
  mean=20,
  var=5,
  nQtlPerChr = 10 # changing this changes effect size? (first one is 10, then 20)
)
...
```{r}
traitArchitecture <- function(pop, fitFunc) {
  initialFitness <- fitFunc(meanG(pop))
...
for (gen in 1:nGens) {
  ...
  for (l in 1:nLoci) {
    ...
    if (FALSE) {
      for (i in 1:nInd(pop)) {
        ...
        if ((fit_post1 > fit_pre)) {#} || (fit_post1 > fit_pre) ) {
          inc <- TRUE
          break
        }
  ...
  pop <- selectCross(pop=pop, trait=fitnessFunc, nInd=popSize*0.8, nCrosses=popSize)
}
par(mar = c(5, 5, 3, 5) + 0.3)
