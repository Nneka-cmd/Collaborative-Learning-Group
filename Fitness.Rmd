...
SP$addTraitA(mean=c(1,1), var=c(0.1,0.1), nQtlPerChr=10, gamma = TRUE, shape = 1)
...
# Figure out a better way to select randomly? or by traitA, traitB?
popA <- selectInd(pop, use="rand", nInd=nInd(pop)*0.25)
popB <- selectInd(pop, use="rand", nInd=nInd(pop)*0.25)
...
p <- fig %>%
  layout(legend=list(title=list(text='Population')),
         ...
         scene = list(xaxis = list(title = "Trait A"),
                      ...
                      zaxis = list(title = "w"),
                      aspectmode='cube')) %>% hide_colorbar()
htmlwidgets::saveWidget(as_widget(p), "~/Downloads/20241212_2adaptivewalks.html")
p
```
