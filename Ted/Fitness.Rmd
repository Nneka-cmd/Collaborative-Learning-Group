...
SP$addTraitA(mean=1, var=0.1, nQtlPerChr=3, gamma = TRUE, shape = 1) # e.g. height
SP$addTraitA(mean=1, var=0.1, nQtlPerChr=3, gamma = TRUE, shape = 1) # e.g. flowering time
#SP$addTraitA(mean=950, var=200, nQtlPerChr = 100, gamma = TRUE, shape = 1)

SP$setVarE(h2=c(0.6, 0.6))
SP$addSnpChip(nSnpPerChr=nMarkers)
foundingPop <- newPop(founders, simParam = SP)
...
fg <- pullQtlGeno(F1)
F2 = self(F1, nProgeny=16)

getCross <- function(F2) {
  snpGeno = pullSnpGeno(F2)
  pheno= data.frame(pheno=F2@pheno)
  snps <- colnames(snpGeno)
  geno <- list()
  for (c in 1:nChr) {
    start <- (c-1)*nMarkers + 1
    end <- nMarkers*c
    chr_snps <- snps[start:end]
    chr_snps <- sapply(chr_snps, function(x) as.numeric(strsplit(x, split="_")[[1]][2]))
    chr_geno <- snpGeno[,start:end]+1
    chr <- as.character(c)  
    geno[[chr]] <- list()
    attributes(obj[[chr]])$class<-"A"
    geno[[chr]]$"data" <- chr_geno
    geno[[chr]]$"map" <- chr_snps
  }
...
out.em <- scanone(cross)
out.hk <- scanone(cross, method="hk")
plot(out.hk)
operm.hk <- scanone(cross, method="hk", n.perm=1000)
summary(operm.hk, alpha=0.05)

#TODO: multiple QTL mapping
#TODO: figure out how to to separate mapping for different traits
```
...
geno = data.frame(
  snp = colnames(geno),
  chr = rep(1:10, each = nMarkers),
  pos = rep(1:nMarkers, 10),
  t(geno - 1)
)
