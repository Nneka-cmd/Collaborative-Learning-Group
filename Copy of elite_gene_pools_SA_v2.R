...
for (i in seq_len(runs)) {
  ...
  # Founder pop simulation will repeat until nind > 10 after stabilizing selection
  # and, ibs values for cis # and trans parents satisfied
  repeat {
  repeat {  
    # create founder pop
  ...
  # create a data frame for selected parents and their meta data
  grps <- data.frame(ids = parents@id,
                     gv1 = round(gv(parents)[,1], 2),
                     trait1 = round(pheno(parents)[,1], 2))
  
  # Pull QTL genotype data for individuals 
  # Extract marker genotypes of QTL loci
  qtl_geno <- pullQtlGeno(parents)
  
  # Use ibs_raw() function to identify parents
  cis_mat <- ibs_raw(geno = qtl_geno, type = "cis")
  trans_mat <- ibs_raw(geno = qtl_geno, type = "trans")
  
  if (all(cis_mat$ibs[1:n.crossesF1] == 1) & all(trans_mat$ibs[1:n.crossesF1] < 1)) {
  ...
  # Save selected parents info
  grp_assign[[i]] <- grps
  
  # myCol <- RColorBrewer::brewer.pal(ngrps, "Set1")
  
  # Save simulated qtl geno data for each run to a folder
  saveRDS(qtl_geno, file = file.path(dir_geno_data,
                                     paste0("run_", i, ".RDS")))
  ...
  for (j in type) {
    ...
    if (j == "cis") {
      ...
      # Use ibs_raw to identify parents
      # cis_mat <- ibs_raw(geno = qtl_geno, type = "cis")
      cis_matx[[i]] <- cis_mat
      
      # Get parents for cis elite crosses
      cross_cis <- as.matrix(cis_mat[1:n.crossesF1, 1:2])
      
      cis_par <- cis_mat[1:n.crossesF1,]
      cis_par$geno_diff <- allele_diff(type = "cis", nselect = n.crossesF1)
      
      cis_parents[[i]] <- cis_par # Save cis elite parents
      
      # Use the ssd_pop() function to advance from F2 to F5
      ...
      # Use ibs_raw() to identify parents
      trans_matx[[i]] <- trans_mat
      
      # Get parents for trans elite crosses
      cross_trans <- as.matrix(trans_mat[1:n.crossesF1, 1:2])
      
      trans_par <- trans_mat[1:n.crossesF1,]
      trans_par$geno_diff <- allele_diff(type = "trans", nselect = n.crossesF1)
      
      trans_parents[[i]] <- trans_par
